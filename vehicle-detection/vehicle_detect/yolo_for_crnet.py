...
def yolo_ocr(original_img,top_cords):
    net = cv2.dnn.readNet( BASE_DIR + "/vehicle_detect/crnet/cr-net.cfg" , BASE_DIR +"/vehicle_detect/crnet/cr-net_last (8).weights",)
    ...
    if len(bboxes) < 1:
        return 'The model was unable to detect any LP characters, try again with a different image!', img
    try:
        ...
        # if label is None:
        #     return 'Not all characters were detected,try again!', car_img
        if len(label) > 1 and len(label) < 9:
            label = 'Characters that were detected are ' + ','.join(class_names[x[4][0]] for x in sorted(bboxes, key=lambda x: x[0]))
            return label, car_img
